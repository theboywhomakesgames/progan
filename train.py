...
def train(generator, discriminator, init_step, loader, total_iter=600000):
    step = init_step # can be 0 = 4, 1 = 8, 2 = 16, 3 = 32, 4 = 64, 5 = 128, 6 = 128
    ...
    for i in pbar:
        ...
        # loss is the mean of discriminator output when input is a real image!
        real_predict = real_predict.mean() \
            - 0.001 * (real_predict ** 2).mean()
        real_predict.backward(mone) # use -1 when it's real image to reduce mean
        ...
        fake_predict.backward(one) # use +1 when it's fake to increase the mean

        ### gradient penalty for D ?????????? I don't understand this part
...
if __name__ == '__main__':
    ...
    parser.add_argument('--path', type=str, default="/home/will/Dev/PROGAN/db/Abstract_gallery", help='path of specified dataset, should be a folder that has one or many sub image folders inside')
    ...
    generator = Generator(in_c=args.channel, input_dim=input_code_size, pixel_norm=args.pixel_norm, tanh=args.tanh).to(device)
    discriminator = Discriminator(feat_dim=args.channel).to(device)
    g_running = Generator(in_c=args.channel, input_dim=input_code_size, pixel_norm=args.pixel_norm, tanh=args.tanh).to(device)
    
    ## you can directly load a pretrained model here
    generator.load_state_dict(torch.load('./trial_test1_2022-02-05_20_59/checkpoint/200000_g.model'))
    g_running.load_state_dict(torch.load('./trial_test1_2022-02-05_20_59/checkpoint/200000_g.model'))
    discriminator.load_state_dict(torch.load('./trial_test1_2022-02-05_20_59/checkpoint/200000_d.model'))
